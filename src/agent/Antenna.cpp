...
const string Antenna::toString(bool detailed) const {
    ...
    if (detailed)
        if (getType() == AntennaType::DIRECTIONAL) {
            result << dumpLocation() << sep << getMNO()->getId() << sep << getMNO()->getMNOName() << sep
                   << getMaxConnections() << sep << getPower() << sep << getAttenuationFactor() << sep
                   << getTypeName() << sep << getSmin() << sep << getQmin() << sep << getSmid() << sep
                   << getSSteep() << sep << getTilt() << sep << getAzimDBBack() << sep << getElevDBBack() << sep
                   << getBeamH() << sep << getBeamV() << sep << getDirection() << sep << getHeight() << sep
                   << m_simConfig->getMap()->getTileNo(getLocation()) << endl;
        }
        else {
            result << dumpLocation() << sep << getMNO()->getId() << sep << getMNO()->getMNOName() << sep
                   << getMaxConnections() << sep << getPower() << sep << getAttenuationFactor() << sep
                   << getTypeName() << sep << getSmin() << sep << getQmin() << sep << getSmid() << sep
                   << getSSteep() << sep << "" << sep << "" << sep << "" << sep << "" << sep << "" << sep << ""
                   << sep << getHeight() << sep << m_simConfig->getMap()->getTileNo(getLocation()) << endl;
        }
    else
        result << ImmovableAgent::toString() << left << setw(15) << m_antennaConfig.getPower() << setw(25)
               << m_antennaConfig.getMaxConnections() << setw(15) << m_antennaConfig.getPle() << setw(15)
...
void Antenna::registerEvent(Event * ev, Point* evtLocation) {
    ...
    stringstream ss;
    if (getMap()->hasGrid())
        ss << ev->toString() << sep << fixed << evtLocation->getX() << sep << evtLocation->getY() << sep
           << getMap()->getTileNo(evtLocation) << endl;
    if (m_file.is_open()) {
        m_file << ss.str();
        m_file.flush();
    }
...
double Antenna::computeSignalDominance(const Point* p) const {
    double result = 0.0;
    const geos_compat::CoordView* c = p->getCoordinate();
    result = computeSignalDominance(geos_compat::toCoordinate(*c));
    return result;
}
...
double Antenna::computePower(const Coordinate c) const {
    double result = 0.0;
    const Coordinate dest = geos_compat::toCoordinate(*getLocation()->getCoordinate());
    if (m_antennaConfig.getType() == AntennaType::OMNIDIRECTIONAL)
...
void Antenna::setLocationWithElevation() {
    Point* p = getLocation();
    Point* newP = geos_compat::createPointRaw(
        getMap()->getGlobalFactory().get(),
        Coordinate(p->getX(), p->getY(), m_antennaConfig.getHeight())
    );
    setLocation(newP);
...
Geometry* Antenna::getCoverageAreaDirectional() const {
    ...
    auto cl = std::make_unique<geos::geom::CoordinateSequence>();
#else
    ...
    for (unsigned i = 0; i < N; i++) {
        double angle = i * (2.0 * utils::PI / static_cast<double>(N));
        double dist = m_rmax;
        ...
        if (k == N) {
            ...
            cout << "Check the parameters of the antenna (Smin/Qmin)!" << endl;
...
#if GEOS_VERSION_MAJOR > 3 || (GEOS_VERSION_MAJOR == 3 && GEOS_VERSION_MINOR >= 11)
    auto lr = this->getMap()->getGlobalFactory()->createLinearRing(std::move(cl));
    auto poly = this->getMap()->getGlobalFactory()->createPolygon(std::move(lr));
    return poly.release();
#else
...
double Antenna::computeSignalStrength(const Point* p) const {
    double result = 0.0;
    const geos_compat::CoordView* c = p->getCoordinate();
    result = computeSignalStrength(geos_compat::toCoordinate(*c));
    return result;
}
...
const string Antenna::getHeader(bool detailed) {
    ...
    char sep = Constants::sep;
    if (detailed)
        result << "t" << sep << "Antenna ID" << sep << "x" << sep << "y" << sep << "MNO ID" << sep << "mno_name"
               << sep << "maxconnection" << sep << "power" << sep << "attenuationfactor" << sep << "type" << sep
               << "Smin" << sep << "Qmin" << sep << "Smid" << sep << "SSteep" << sep << "tilt" << sep
               << "azim_dB_back" << sep << "elev_dB_back" << sep << "beam_h" << sep << "beam_v" << sep
               << "direction" << sep << "z" << sep << "Tile ID" << endl;
    else
        result << left << setw(15) << "Antenna ID" << setw(15) << " X " << setw(15) << " Y " << setw(15) << " Power "
               << setw(15) << "Max Connections" << setw(20) << "Attenuation Factor" << setw(15) << "MNO ID" << endl;
    return result.str();
}
...
EventConfig* Antenna::buildEventConfig(EventType evType, EventCode code, HoldableAgent* device) {
    ...
    if (evType == EventType::CELLID) {
        ...
        unsigned int ta = static_cast<unsigned int>(-1);
        if (m_antennaConfig.getNetworkType() == NetworkType::_3G) {
            ta = static_cast<unsigned int>(dist / Antenna::delta3G);
            if (ta > Antenna::MAXTA3G)
                ta = Antenna::MAXTA3G;
        } else if (m_antennaConfig.getNetworkType() == NetworkType::_4G) {
            ta = static_cast<unsigned int>(dist / Antenna::delta4G);
