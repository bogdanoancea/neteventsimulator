...
Point* ManhattanDisplacement::generateNewLocation(Point * initLocation) {
    Point* result;
    //const geos::geom::CoordinateXY* initC = initLocation->getCoordinate();
	const geos_compat::CoordView* initC = initLocation->getCoordinate();
    double x = initC->x;
    double y = initC->y;
    ...
    if (m_simConfig->getClock()->getCurrentTime() ==
        m_simConfig->getClock()->getInitialTime() + m_simConfig->getClock()->getIncrement()) {
        current = closestCorner(Coordinate(*initC));
        //result = m_simConfig->getMap()->getGlobalFactory()->createPoint(current).release();
		result = geos_compat::createPointRaw(m_simConfig->getMap()->getGlobalFactory().get(), current
);
...
ManhattanDisplacement::STATE ManhattanDisplacement::checkStatus(Point* location) const {
    ...
    if (doubleCompare(fmod(x - m_manhattanScenario->getXOrigin(), xManhattanStep), 0.0) &&
        doubleCompare(fmod(y - m_manhattanScenario->getYOrigin(), yManhattanStep), 0.0)) {
        result = STATE::ONCORNER;
    }
    else if (doubleCompare(fmod(x - m_manhattanScenario->getXOrigin(), xManhattanStep), 0.0) ||
             doubleCompare(fmod(y - m_manhattanScenario->getYOrigin(), yManhattanStep), 0.0)) {
        result = STATE::ONEDGE;
    }
    else {
